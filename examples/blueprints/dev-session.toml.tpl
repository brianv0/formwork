...
# ultimate transparency check. If `cargo test` and a full edit loop run clean in here with zero
...
# Deliberate widenings: the model API key through the launcher strip, and this agent's
...
# compiled-in catalog floor; driving Docker from inside this session cannot work, run
# `just test-linux` from an unconfined shell instead. Only the dev-specific hole remains here:
subtract = [
    # Installed binaries run UNSANDBOXED later; don't let a confined agent rewrite them (the
    # execution-vector concern). cargo build doesn't write here; cargo install would.
    "~/.cargo/bin/**",
]

# Tamper vectors: write-denied but readable. Even developing Formwork, the agent must not
