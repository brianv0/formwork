...
- **Spike gates mechanism.** The feed choice (fs_usage vs SBPL report vs ASID auditpipe) is unproven
  ...
  the spike resolves.
- **Draft test IDs sit above the highest landed number.** First drafted at `FW-E2E-065`..067 (above
  #18's `FW-E2E-062..064`), but #19 landed the MCP-shading tests at `FW-E2E-065`..067 and gateway
  tests through `FW-E2E-069`, so the unlanded draft is renumbered up to `FW-E2E-070`..072 (never the
  landed spec; constitution Requirements & identifiers). See `fep-4.md` §4/§6.
...
- `FW-INV12` recording floor — no recording can observe, propose, or synthesize a credential
  location.
- Tests `FW-E2E-070` (round-trip), `FW-E2E-071` (floor), `FW-E2E-072` (no-feed fail-loud).
...
- `FW-E2E-070` round-trip; `FW-E2E-071` floor; `FW-E2E-072` no-feed fail-loud (§6).
...
- **Real-boundary (macOS, no mocking the kernel — constitution Testing):**
  - `FW-E2E-070` — spawn a workload under the floor-only policy; observed opens synthesize a
  ...
  - `FW-E2E-071` — a workload reading a credential during recording is denied at the kernel and the
    credential is absent from proposal and synthesized Blueprint.
  - `FW-E2E-072` — on `trace-feed: none`, `learn --permissive` fails loud and writes nothing.
