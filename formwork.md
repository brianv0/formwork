...
<a id="fw-e2e-064"></a>**FW-E2E-064: Short-lived workload denials are captured ([FW-DISC1](#fw-disc1)/[FW-DISC2](#fw-disc2)).** A learning run whose workload dies on its first denial (`cat` of an ungranted file — exiting in well under a second, the canonical discovery shape). Collection is anchored to the run start, held open for a minimum settle window — an empty read repeated is never trusted before it — and polled to quiescence under a cap. The macOS unified-log feed's persistence latency has an unbounded tail, so a lone run can flush past the settle window and miss its record; that miss is safe (the next learn run re-observes it, [FW-INV10](#fw-inv10)), which is why the property is a **capture rate across independent runs**, not a per-run certainty no post-hoc log feed can offer. Pass: the denied path appears in the proposal in the large majority of runs (a capture-rate floor). Fail: the capture rate falls below the floor — the feed is dropping records the kernel produced, not merely reading a window it had not yet flushed.
